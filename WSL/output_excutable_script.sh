...
if [ -e $HOME/dotfiles/WSL/setup.sh ]; then
  if [ $# -ne 0 ]; then
    if [ $1 == "-h" -o $1 == "--help" ]; then
      echo "usage: $ bash output_executable_script.sh [executable=BASH, VIM, PYENV]"
      ...
      do
        ...
        j=$(( i+1 ))
        args=`echo $@ | cut -d " " -f$i`
        if [ $args == "PYENV" ] || [ $args == "BASH" ] || [ $args == "VIM" ]; then
    ...
    echo "[*] Output executable setup script to $HOME/setup.sh"
    cat ~/dotfiles/WSL/setup.sh | egrep -v "^.*?BASH" | egrep -v "^.*?PYENV" | egrep -v "^.*?VIM" > $HOME/setup.sh
    echo "[!] Success output"
  fi
fi
